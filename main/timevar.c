/* Timing variables for measuring compiler performance.
   ...
   [COREMU XXX]
   Modified from GCC for COREMU multicore emulation infrastructure. NO
   backward compatibility is provided, and we always use the latest
   interfaces.

   contact: Xi Wu (wuxi@fudan.edu.cn) */
...
/* Prefer times to getrusage to clock (each gives successively less
   ...
   COREMU XXX: use times is enough, because we have this interface */
...
#else
    cm_assert(false, "cannot use TIMES function?!");
#endif
}
...
timevar_push_1 (timevar_id_t timevar)
{
    ...
    cm_assert (!tv->standalone, "cannot push a standalone timer");
    ...
    if (unused_stack_instances != NULL)
    {
        ...
        context = qemu_mallocz(sizeof (struct timevar_stack_def));
...
timevar_pop_1 (timevar_id_t timevar)
{
    ...
    cm_assert (&timevars[timevar] == stack->timevar,
               "two timevars should be equal");
...
timevar_start (timevar_id_t timevar)
{
    ...
    /* Don't allow the same timing variable to be started more than
       once.  */
    cm_assert (!tv->standalone,
               "tv cannot be standalone");
    tv->standalone = 1;
